...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_GET['ajax_calendar'])) {
    ...
    // Server-side guard: schedule is required
    if (empty($pickup_schedule_id)) {
        $_SESSION['schedule_required'] = true;
...
<div class="container">
    <div class="row">
        <div class="col-md-8">
            ...
            // Show SweetAlert if schedule was missing on last submit
            if (isset($_SESSION['schedule_required']) && $_SESSION['schedule_required']) {
                echo "<script>document.addEventListener('DOMContentLoaded', function(){ Swal.fire({ icon: 'warning', title: 'Schedule Required', text: 'Please select a claiming schedule before submitting your request.' }); });</script>";
                unset($_SESSION['schedule_required']);
            }
            ?>
...
");
                     ...
                     <div class="mb-2 position-relative">
                                ...
                                <div class="form-text small">Please select a schedule to coordinate with the captain.</div>
                            </div>
...
<script>
    ...
    // Add form validation to ensure a purpose and schedule are selected
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            const docTypeSelect = document.getElementById('doc_type_id');
            const scheduleId = document.getElementById('pickup_schedule_id').value;
            const purposeSection = document.getElementById('purpose-section');
            ...
            if (!scheduleId || scheduleId.trim() === '') {
                ...
                Swal.fire({
                    ...
                    text: 'Please select a claiming schedule before submitting your request.'
                });
                return false;
            }
